...
def user_login(request):
	...
	if request.method == 'POST':
		...
		# We use the request.POST.get('<variable>') as opposed
		# to request.POST['<variable>'] because the
		# request.POST.get('<variable>') returns None if the 
		# value doe snot exist, while request.POST['<variable']
		# will raise a KeyError exception.
		username = request.POST.get('username')
		password = request.POST.get('password')

		# Use Django's machinery to attempt to see if hte username/passsword
		...
		if user:
			...
			if user.is_active:
				...
				# An inactive accout was used - no logging in!
